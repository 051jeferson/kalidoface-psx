...
- `perfAuto` sheds **tracking rate only**, and only while frames are actually being dropped; `PSX.perf()` is the readout. It learns the best animation-frame interval the machine has delivered rather than assuming a display rate — a hard-coded 16.7 would be a cap tuned to one monitor. Measure that interval in `frame()` **before** the render cap returns: a capped render answers a different question. Shed from the rate the tracker is actually achieving (`trackHz`), not from the ceiling — holistic never reaches 60, so shedding from 60 spends several decisions cutting a rate nothing was using while the stutter continues. And stop shedding once a cut stops buying frames back: past that point the load is another process and the only thing left to starve is the avatar. Do not throttle the render rate or the pixel ratio from here — both reframe or degrade the shot that is being captured, and neither is where the CPU goes.
- Per-frame work is measured against the *rendered* frame rate, not the inference rate: the retarget runs on every rendered frame while the landmarks behind it change perhaps twenty times a second. Anything derived from landmarks alone (`sideHitsFace`, `imgHeadNear`, `faceLim2`) is memoised on `imgSeq`, and anything measured off the model that does not move (`armSeg`, `headHeight`) is re-read on a `MEASURE_EVERY` interval rather than per frame — every `worldPos` walks that bone's whole parent chain re-multiplying matrices. `imgSeq` and not `poseSeq`: `poseSeq` is bumped after the world landmarks are accepted, and the image set is written by `hands()` before `pose()` runs and again on frames whose world set is rejected. Anything that moves with the pose - `mb`, the head bone's own position - stays live.
